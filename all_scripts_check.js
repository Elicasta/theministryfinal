...
function adminSelectLesson(slug){
  ...
  try{ setTimeout(()=>{ if(typeof window.refreshLessonStreamAdminPanel==='function') window.refreshLessonStreamAdminPanel(); },80); }catch(e){}
}
...
(function(){
  ...
  function cfgFromApiRow(row){
    if(!row) return null;
    return {
      status:row.status||'starting-soon',
      provider:row.provider||'youtube',
      embedUrl:row.embed_url || row.embedUrl || '',
      title:row.title||'The Ministry Live',
      serviceLabel:row.service_label || row.serviceLabel || DEFAULT_STREAM.serviceLabel,
      syncDelaySeconds:row.sync_delay_seconds ?? row.syncDelaySeconds ?? 0,
      updatedAt:row.updated_at || row.updatedAt || null,
      source:row.source || row.scope || ''
    };
  }
  function lessonDisplayLabel(slug){
    const meta=(window.HOME_LESSON_META||HOME_LESSON_META||{})[slug||currentLessonSlug()];
    return meta ? `${meta.label} · ${meta.title}` : (slug||currentLessonSlug());
  }
  ...
  async function fetchOnlineConfig(){
    ...
    try{
      ...
      if(data&&data.config) applyOnlineConfig(cfgFromApiRow(data.config));
  ...
  async function saveStreamConfigFromAdmin(){
    ...
    const saveGlobal=!!(($('stream-save-global')||{}).checked);
    const cfg={series_slug:currentSeriesSlug(),lesson_slug:currentLessonSlug(),status,provider,embedUrl:embed,title,serviceLabel,syncDelaySeconds:delay,save_global:saveGlobal,updatedAt:new Date().toISOString()};
    applyOnlineConfig(cfg);
    const msg=$('stream-admin-status'); if(msg) msg.textContent=`Saving stream source for ${lessonDisplayLabel(currentLessonSlug())}...`;
    try{
      ...
      if(msg) msg.textContent=(r&&r.ok)?(saveGlobal?'Saved to this lesson and global fallback.':'Saved to this lesson only.'):'Saved locally. API save did not confirm.';
      setTimeout(refreshLessonStreamAdminPanel,80);
  ...
  async function refreshLessonStreamAdminPanel(){
    const box=$('stream-lesson-saved');
    const msg=$('stream-admin-status');
    try{
      const r=await fetch(streamConfigApiUrl(),{cache:'no-store'});
      const data=await r.json();
      const lessonCfg=cfgFromApiRow(data.lesson_config);
      const globalCfg=cfgFromApiRow(data.global_config);
      const chosen=lessonCfg || {...DEFAULT_STREAM,embedUrl:'',status:'starting-soon',source:'none'};
      if($('stream-embed-url')) $('stream-embed-url').value=chosen.embedUrl||'';
      if($('stream-status')) $('stream-status').value=chosen.status||'starting-soon';
      if($('stream-sync-delay')) $('stream-sync-delay').value=Number(chosen.syncDelaySeconds)||0;
      if($('stream-title')) $('stream-title').value=chosen.title||'The Ministry Live';
      if($('stream-service-label')) $('stream-service-label').value=chosen.serviceLabel||DEFAULT_STREAM.serviceLabel;
      if(box){
        const lessonLine=lessonCfg ? `<b>Saved for ${esc(lessonDisplayLabel(currentLessonSlug()))}</b><div class="stream-lesson-url">${esc(lessonCfg.embedUrl||'No URL saved.')}</div>` : `<b>No stream saved for ${esc(lessonDisplayLabel(currentLessonSlug()))}</b><div class="stream-lesson-url">This lesson will use the global fallback online until you save a lesson stream.</div>`;
        const globalLine=globalCfg && globalCfg.embedUrl ? `<div style="margin-top:7px">Global fallback: <span class="stream-lesson-url">${esc(globalCfg.embedUrl)}</span></div>` : `<div style="margin-top:7px">Global fallback: none saved.</div>`;
        box.innerHTML=lessonLine+globalLine;
      }
      if(msg) msg.textContent=lessonCfg?'Loaded lesson-specific stream.':'No lesson stream yet. Paste a URL and Save Stream.';
    }catch(e){
      if(box) box.innerHTML=`<b>${esc(lessonDisplayLabel(currentLessonSlug()))}</b><div class="stream-lesson-url">Could not load saved stream for this lesson.</div>`;
      if(msg) msg.textContent='Could not load lesson stream state.';
    }
  }
  window.refreshLessonStreamAdminPanel=refreshLessonStreamAdminPanel;

  function ensureStreamAdminPanel(){
    const right=document.querySelector('#admin-hub .ctrl-right [style*="overflow-y"]') || document.querySelector('#admin-hub .ctrl-right');
    if(!right || $('stream-admin-panel')) return;
    const cfg={...DEFAULT_STREAM,embedUrl:''};
    const panel=document.createElement('div');
    panel.id='stream-admin-panel';
    panel.className='stream-admin-panel';
    panel.innerHTML=`<div class="ctrl-ag-lbl">Online Stream</div>
      <div class="stream-lesson-saved" id="stream-lesson-saved"><b>${esc(lessonDisplayLabel(currentLessonSlug()))}</b><div class="stream-lesson-url">Loading saved stream for this lesson...</div></div>
      ...
      <label class="stream-global-toggle"><input id="stream-save-global" type="checkbox" /> Also update global fallback</label>
      <div class="stream-admin-actions"><button class="ctrl-ab red-btn" onclick="saveStreamConfigFromAdmin()">Save Stream</button><button class="ctrl-ab" onclick="refreshLessonStreamAdminPanel()">Reload Lesson Stream</button><button class="ctrl-ab" onclick="window.open('/online?lesson='+encodeURIComponent((window.LESSON_SLUG||'lesson-1')),'_blank')">Open /online</button><button class="ctrl-ab" onclick="window.open('/Espanol?lesson='+encodeURIComponent((window.LESSON_SLUG||'lesson-1')),'_blank')">Open /Espanol</button></div>
      <div class="stream-admin-status" id="stream-admin-status">Each lesson keeps its own saved YouTube link.</div>`;
    right.insertBefore(panel,right.firstChild);
    const st=$('stream-status'); if(st) st.value=cfg.status||'starting-soon';
    refreshLessonStreamAdminPanel();
  }
...
(function(){
  ...
  const qs=()=>new URLSearchParams(window.location.search);
  const isSpanishRoute=()=>route()==='espanol';
  const wantsOnline=()=>route()==='online'||(isSpanishRoute()&&/^(1|true|online)$/i.test(qs().get('online')||qs().get('view')||''));
  const currentLesson=()=>{try{return requestedLessonSlug()}catch(e){return window.LESSON_SLUG||'lesson-1'}};
  function setHTML(id,html){const el=$(id); if(el) el.innerHTML=html;}
  function setText(sel,text){const el=document.querySelector(sel); if(el) el.textContent=text;}
  function strip(html){const d=document.createElement('div');d.innerHTML=String(html||'');return (d.textContent||'').replace(/\s+/g,' ').trim();}

  // Lesson date default: keep every route on date-based current lesson unless explicit ?lesson= is present.
  function forceCurrentLessonIfNoQuery(){
    const p=qs();
    if(p.get('lesson')||p.get('l')) return;
    const slug=currentLesson();
    if(slug && slug!==window.LESSON_SLUG && typeof applySelectedLesson==='function'){
      try{applySelectedLesson(slug);}catch(e){}
    }
  }

  // Login note and final routing override. This bypasses the earlier v47 wrapper that redirected Spanish too early.
  function patchLoginFinal(){
    const ac=$('mo-access-code');
    if(ac && !$('access-case-note-v49')){
      const note=document.createElement('div');
      note.id='access-case-note-v49';
      note.className='access-case-note-v49';
      note.innerHTML='* access code is <b>all lowercase</b>';
      ac.appendChild(note);
    }
    if(window.__v49LoginPatched) return;
    window.__v49LoginPatched=true;
    window.checkPw=async function(){
      const err=$('mo-err'); if(err){err.classList.remove('on'); err.textContent='Incorrect code. Try again.';}
      const pw=String(($('mo-pw')||{}).value||'');
      const name=String(($('mo-name')||{}).value||'').trim();
      const emailEl=$('mo-email');
      const email=String((emailEl||{}).value||'').trim().toLowerCase();
      const wantsO=!!(($('mo-online')||{}).checked);
      const wantsEs=!!(($('mo-espanol')||{}).checked);
      if(pw===window.PW_ADMIN || pw==='ministryadmin'){
        localStorage.setItem('tm_a','1'); localStorage.setItem('tm_u','1');
        if(typeof activateAdmin==='function') activateAdmin();
        if(typeof closeModal==='function') closeModal();
      ...
      if(!(pw===window.PW_ATTENDEE || pw==='ministry2026')){ if(err) err.classList.add('on'); return; }
      if(!name){ if(err){err.textContent='Enter your name.';err.classList.add('on');} return; }
      if(emailEl && !/^\S+@\S+\.\S+$/.test(email)){ if(err){err.textContent='Enter a valid email.';err.classList.add('on');} return; }
      localStorage.setItem('tm_u','1'); localStorage.setItem('tm_name',name); if(email) localStorage.setItem('tm_email',email);
      if(typeof setUserName==='function') setUserName(name);
      // Save attendee silently when that helper exists, but do not block routing on it.
      try{ if(typeof saveAttendee==='function' && email) await saveAttendee(name,email); }catch(e){console.warn('attendee save skipped',e);}
      const lesson=encodeURIComponent(window.LESSON_SLUG||currentLesson()||'lesson-1');
      if(wantsEs && wantsO){ window.location.href='/Espanol?online=1&lesson='+lesson; return; }
      if(wantsEs){ window.location.href='/Espanol?lesson='+lesson; return; }
      if(wantsO){ window.location.href='/online?lesson='+lesson; return; }
      if(typeof closeModal==='function') closeModal();
      if(typeof showHub==='function') showHub();
    };
  }

  function activateSpanishParticipant(){
    if(!isSpanishRoute() || wantsOnline()) return;
    window.TM_LANG_ES=true;
    document.body.classList.add('tm-spanish-mode');
    document.body.classList.remove('online-viewer-mode');
    const ov=$('online-viewer'); if(ov) ov.classList.remove('on');
    const ssl=$('ssl'), sh=$('sh'), sc=$('sc');
    ['admin-hub','questionnaire'].forEach(id=>{const el=$(id); if(el) el.classList.remove('on');});
    if(sc) sc.classList.remove('on');
    if(ssl) ssl.classList.remove('on');
    if(sh) sh.classList.add('on');
    localStorage.setItem('tm_u','1');
    translateStaticSpanish();
    rebuildSpanishSlides();
  }
  function translateStaticSpanish(){
    const title=document.querySelector('.sc-title'); if(title) title.innerHTML='<span class="tt">EL</span><span class="tm">MINISTERIO</span>';
    setText('.sc-over','LLAMADOS CERCA. ENVIADOS LEJOS.');
    setText('.sc-sub','UNA SERIE DE MATEO CAPÍTULO 10');
    setText('.sc-enter','ENTRAR A LA SESIÓN');
    setText('.sc-presenter','PRESENTADOR');
  ...
  function rebuildSpanishSlides(){
    try{
      if(typeof buildSlides==='function'){
        buildSlides($('ss-slides'), window.LESSON1_SLIDES||LESSON1_SLIDES||[]);
        if(typeof renderDots==='function') renderDots((window.LESSON1_SLIDES||LESSON1_SLIDES||[]).length);
        if(typeof goTo==='function') goTo(Math.max(0,Number(window.curSlide||curSlide||0)));
      }
    }catch(e){console.warn('Spanish slide rebuild skipped',e);}
  }

  // Online page ordering: video, lower-third/current cue, current slide, responses. Desktop keeps responses side/sticky.
  function fixOnlineLayout(){
    const main=document.querySelector('.online-main-column');
    const side=document.querySelector('.online-side-column');
    if(!main||!side) return;
    let lower=$('online-lower-card');
    if(!lower){
      lower=document.createElement('section');
      lower.id='online-lower-card';
      lower.className='online-lower-card';
      lower.innerHTML='<div class="online-lower-kicker" id="online-lower-kicker">On Screen</div><div class="online-lower-title" id="online-lower-title">Waiting for slide cue.</div><div class="online-lower-ref" id="online-lower-ref">The live lower-third information will appear here.</div><a class="online-watch-again" id="online-watch-again" target="_blank" rel="noopener">Watch Again</a>';
    }
    const video=$('online-video-card'); if(video && video.parentNode===main && lower.parentNode!==main) video.insertAdjacentElement('afterend',lower);
    const slideCard=Array.from(document.querySelectorAll('.online-card')).find(c=>c.querySelector('#online-current-slide'));
    if(slideCard){ slideCard.id='online-slide-shell'; if(slideCard.parentNode!==main) main.insertBefore(slideCard, lower.nextSibling); }
    const response=Array.from(document.querySelectorAll('.online-card')).find(c=>c.querySelector('#online-respond-label'));
    if(response){ response.id='online-respond-shell'; if(window.innerWidth>=900 && response.parentNode!==side) side.insertBefore(response, side.firstChild); }
    renderLowerThirdV49();
  }
  function renderLowerThirdV49(){
    const slides=window.LESSON1_SLIDES||[];
    const state=window.onlineViewerState||{};
    const i=Math.max(0,Math.min(Number(state.slide||window.curSlide||0),Math.max(0,slides.length-1)));
    const s=slides[i]||{};
    const title=strip(s.hl||s.title||s.text||s.ref||'The Ministry');
    const ref=strip(s.ref||s.tk||((window.LESSON_DATA||{}).text)||'');
    setHTML('online-lower-kicker',(window.TM_LANG_ES||isSpanishRoute())?'En Pantalla':'On Screen');
    setHTML('online-lower-title',title||'The Ministry');
    setHTML('online-lower-ref',ref||'');
    const wa=$('online-watch-again');
    const cfg=(window.onlineViewerState&&window.onlineViewerState.config)||{};
    const url=String(cfg.embedUrl||'');
    if(wa && url && /ended|replay/i.test(String(cfg.status||''))){
      wa.href=url.includes('/embed/')?url.replace('/embed/','/watch?v='):url;
      wa.textContent=(window.TM_LANG_ES||isSpanishRoute())?'Ver Otra Vez':'Watch Again';
      wa.classList.add('on');
    }else if(wa){wa.classList.remove('on');}
  }

  // Mobile full Bible bank. Kept behind a deliberate BANK button, not near Kill Poll.
  function installMobileBibleBank(){
    const actions=document.querySelector('#mobile-mode .mm-actions');
    const main=document.querySelector('#mobile-mode .mm-main');
    if(!actions||!main||$('mm-bank-switch')) return;
    const btn=document.createElement('button');
    btn.id='mm-bank-switch'; btn.className='mm-action mm-bank-switch'; btn.textContent='Bible Bank';
    btn.onclick=function(){document.body.classList.toggle('mm-show-bank'); renderMobileBibleBank();};
    actions.appendChild(btn);
    const bank=document.createElement('div'); bank.id='mm-bank-full'; bank.className='mm-bank-full';
    bank.innerHTML='<div class="mm-v-head"><span>Bible Bank</span><button class="mm-action" onclick="document.body.classList.remove(\'mm-show-bank\')">Close</button></div><div class="mm-bank-controls"><select id="mm-bank-book"></select><select id="mm-bank-chapter"></select></div><div class="mm-bank-list" id="mm-bank-list"></div>';
    main.appendChild(bank);
    setTimeout(renderMobileBibleBank,50);
  }
  window.renderMobileBibleBank=function(){
    const data=window.BIBLE_BANK_CURRENT||{}; const books=(data.books)||{};
    const bookSel=$('mm-bank-book'), chSel=$('mm-bank-chapter'), list=$('mm-bank-list');
    if(!bookSel||!chSel||!list) return;
    const bookNames=Object.keys(books);
    if(!bookNames.length){list.innerHTML='<div class="online-empty">No full chapter bank for this lesson.</div>';return;}
    if(!bookNames.includes(bookSel.value)){bookSel.innerHTML=bookNames.map(b=>`<option>${b}</option>`).join(''); bookSel.value=bookNames[0];}
    const chapters=Object.keys(books[bookSel.value]||{});
    if(!chapters.includes(chSel.value)){chSel.innerHTML=chapters.map(c=>`<option>${c}</option>`).join(''); chSel.value=chapters[0];}
    bookSel.onchange=()=>{chSel.value=''; renderMobileBibleBank();}; chSel.onchange=renderMobileBibleBank;
    const verses=(books[bookSel.value]&&books[bookSel.value][chSel.value])||[];
    list.innerHTML=verses.map(([ref,txt])=>`<button class="mm-bank-card" onclick="pushRawScripture({ref_en:'${String(ref).replace(/'/g,"&#39;")}',text_en:\`${String(txt).replace(/`/g,"'")}\`,ref_es:'${String(ref).replace(/'/g,"&#39;")}',text_es:''});mobileFeedback&&mobileFeedback('verse',this)"><div class="ref">${ref}</div><div class="txt">${txt}</div></button>`).join('');
  };
  const oldOpenMobile=window.openMobileMode;
  if(oldOpenMobile){window.openMobileMode=function(){const r=oldOpenMobile.apply(this,arguments);setTimeout(installMobileBibleBank,60);return r;};}

  // Keep online current slide from falling back to the logo when there is no live cue yet.
  const oldEnter=window.enterOnlineViewer;
  if(oldEnter && !oldEnter.__v49){
    const wrapped=function(){
      forceCurrentLessonIfNoQuery();
      if(isSpanishRoute() && !wantsOnline()){activateSpanishParticipant(); return;}
      if(isSpanishRoute()) window.TM_LANG_ES=true;
      const r=oldEnter.apply(this,arguments);
      setTimeout(()=>{fixOnlineLayout(); try{if(window.renderOnlineAll) window.renderOnlineAll();}catch(e){} renderLowerThirdV49();},120);
      return r;
    };
    wrapped.__v49=true; window.enterOnlineViewer=wrapped;
  }
  const oldHandle=window.handleOnlineMessage;
  if(oldHandle && !oldHandle.__v49){
    const wrapped=function(){const r=oldHandle.apply(this,arguments);setTimeout(()=>{fixOnlineLayout();renderLowerThirdV49();},40);return r;};
    wrapped.__v49=true; window.handleOnlineMessage=wrapped;
  }

  document.addEventListener('DOMContentLoaded',()=>{
    forceCurrentLessonIfNoQuery();
    patchLoginFinal(); setTimeout(patchLoginFinal,250); setTimeout(patchLoginFinal,900);
    installMobileBibleBank();
    if(isSpanishRoute() && !wantsOnline()) setTimeout(activateSpanishParticipant,80);
    if(wantsOnline()) setTimeout(fixOnlineLayout,220);
  });
})();
...
(function(){
  ...
  const route=()=>window.location.pathname.replace(/^\/+|\/+$/g,'').toLowerCase();
  const qs=()=>new URLSearchParams(window.location.search);
  const today=()=>{try{return isoLocalDate(new Date());}catch(e){return new Date().toISOString().slice(0,10);}};
  const wantsOnline=()=>route()==='online'||(route()==='espanol'&&/^(1|true|online)$/i.test(qs().get('online')||qs().get('view')||''));
  const isSpanish=()=>route()==='espanol'||document.body.classList.contains('tm-spanish-mode')||window.TM_LANG_ES;
  const strip=html=>{const d=document.createElement('div');d.innerHTML=String(html||'');return (d.textContent||'').replace(/\s+/g,' ').trim();};
  ...
  const dateActive=slug=>{const meta=(window.HOME_LESSON_META||HOME_LESSON_META||{})[slug]; if(!meta) return false; const map={'lesson-1':'2026-06-18','lesson-2':'2026-06-25','lesson-3':'2026-07-02','lesson-4':'2026-07-09'}; return (map[slug]||'9999-12-31')<=today();};
  function currentSlug(){try{return requestedLessonSlug();}catch(e){return window.LESSON_SLUG||'lesson-1';}}

  /* One lowercase note only. */
  function fixAccessCaseNote(){
    const ac=$('mo-access-code'); if(!ac) return;
    const old=$('access-case-note'); if(old) old.remove();
    document.querySelectorAll('#access-case-note-v49').forEach(n=>n.remove());
    if(!$('access-case-note-v50')){
      const note=document.createElement('div'); note.id='access-case-note-v50'; note.className='access-case-note-v50'; note.innerHTML='* access code is <b>all lowercase</b>';
      ac.appendChild(note);
    }
  }

  /* Spanish participant page should feel Spanish, not English shell with Spanish slides only. */
  const ES_META={
    'lesson-1':{label:'Lección 1',dateShort:'18 de junio',dateLong:'18 de junio, 2026',title:'El Precio de Ser Enviado',text:'Mateo 10:1-10',slides:'15 diapositivas',tagline:'El ministerio no comienza con una plataforma. Comienza con un llamado.',reflectionTitle:'Examen Personal',reflectionMeta:'6 secciones · 17 preguntas'},
    'lesson-2':{label:'Lección 2',dateShort:'25 de junio',dateLong:'25 de junio, 2026',title:'La Disciplina del Enviado',text:'Mateo 10:11-16',slides:'20 diapositivas',tagline:'Jesús no solo envía discípulos. Los instruye en cómo moverse.',reflectionTitle:'La Disciplina del Enviado',reflectionMeta:'10 preguntas de reflexión'},
    'lesson-3':{label:'Lección 3',dateShort:'2 de julio',dateLong:'2 de julio, 2026',title:'La Formación de un Ministro',text:'1 Timoteo 4:12-16 · 2 Timoteo 1, 2, 4',slides:'15 diapositivas',tagline:'La formación de un ministro es la formación de un vaso seguro para la obra santa.',reflectionTitle:'La Formación de un Ministro',reflectionMeta:'13 preguntas de reflexión'},
    'lesson-4':{label:'Lección 4',dateShort:'9 de julio',dateLong:'9 de julio, 2026',title:'La Recompensa de la Obra',text:'Mateo 10:34-42',slides:'Próximamente',tagline:'Próximamente.',reflectionTitle:'Reflexión',reflectionMeta:'Próximamente'}
  };
  function translateSpanishParticipant(){
    if(route()!=='espanol' || wantsOnline()) return;
    window.TM_LANG_ES=true; document.body.classList.add('tm-spanish-mode');
    const active=window.LESSON_SLUG||currentSlug(); const meta=ES_META[active]||ES_META['lesson-3'];
    setHTML('.sc-title','<span class="tt">EL</span><span class="tm">MINISTERIO</span>');
    setText('.sc-over','LLAMADOS CERCA. ENVIADOS LEJOS.');
    setText('.sc-sub','UNA SERIE DE MATEO CAPÍTULO 10');
    setText('.sc-tag',meta.tagline);
    setText('.sc-enter','ENTRAR A LA SESIÓN');
    setText('.sc-presenter','PRESENTADOR');
    document.querySelectorAll('.sc-si').forEach(el=>{
      const txt=el.textContent||''; const n=(txt.match(/Lesson\s+(\d)/i)||[])[1]; const key=n?'lesson-'+n:null; if(key&&ES_META[key]) el.textContent=`${ES_META[key].label} · ${ES_META[key].dateShort}`;
    });
    setHTML('.hub-title','EL <span>MINISTERIO</span>');
    setText('.hub-k','SERIE DE MATEO 10');
    setText('.hub-sub','UNA SERIE DE MATEO CAPÍTULO 10');
    setText('.hub-ref',meta.tagline);
    const nav=[['#slides','DIAPOSITIVAS'],['#reflect','REFLEXIONAR'],['#ask','PREGUNTAR']];
    nav.forEach(([href,txt])=>setText(`.hub-nav a[href="${href}"]`,txt));
    document.querySelectorAll('button').forEach(b=>{
      const t=(b.textContent||'').trim().toLowerCase();
      if(t==='open slides →') b.textContent='ABRIR DIAPOSITIVAS →';
      if(t==='return to session') b.textContent='REGRESAR A LA SESIÓN';
      if(t==='reflect →') b.textContent='REFLEXIONAR →';
      if(t==='ask a question') b.textContent='HACER UNA PREGUNTA';
      if(t==='presenter') b.textContent='PRESENTADOR';
      if(t==='open') b.textContent='ABRIR';
      if(t==='live') b.textContent='EN VIVO';
    });
    setText('.hub-lbl','PROGRESO DE LA SERIE · CADA JUEVES');
    document.querySelectorAll('.s-cell').forEach(el=>{
      const txt=el.textContent||''; const n=(txt.match(/0?(\d)\s+·/)||[])[1]; const key=n?'lesson-'+n:null; if(key&&ES_META[key]){
        const cd=el.querySelector('.cd'); if(cd) cd.textContent=ES_META[key].dateShort;
        const body=Array.from(el.childNodes).find(n=>n.nodeType===1&&n.tagName==='DIV'&&!n.classList.contains('cd'));
        if(body) body.innerHTML=`<span class="cn">${String(n).padStart(2,'0')} ·</span> ${ES_META[key].title}`;
      }
    });
    const cards=document.querySelectorAll('.hub-body > .lcard');
    if(cards[0]) cards[0].innerHTML=`<div><div class="lc-wk">${meta.label} · ${meta.dateLong}</div><div class="lc-ti">${meta.title}</div><div class="lc-rf">${meta.text} · ${meta.slides}</div></div><div style="display:flex;align-items:center;gap:12px"><div class="lc-bg live">En Vivo</div><div style="color:var(--mu)">→</div></div>`;
    const q=$('q-hub-card'); if(q){
      const wk=q.querySelector('.lc-wk'); if(wk) wk.textContent=`Final de ${meta.label}`;
      const ti=q.querySelector('.lc-ti'); if(ti) ti.textContent=meta.reflectionTitle;
      const rf=q.querySelector('.lc-rf'); if(rf) rf.textContent=meta.reflectionMeta;
      const badge=q.querySelector('.lc-bg'); if(badge) badge.textContent=dateActive(active)?'ABRIR':'SE ABRE AL CIERRE';
    }
  }

  /* Make lesson access date-driven everywhere and not localStorage-driven. */
  const oldShowHub=window.showHub;
  if(oldShowHub && !oldShowHub.__v50){
    window.showHub=function(){
      if(!qs().get('lesson')&&!qs().get('l')){try{applySelectedLesson(currentSlug());}catch(e){}}
      const r=oldShowHub.apply(this,arguments);
      setTimeout(()=>{installWatchAgainHub(); translateSpanishParticipant(); unlockByDateUI();},30);
      return r;
    }; window.showHub.__v50=true;
  }
  function unlockByDateUI(){
    const active=window.LESSON_SLUG||currentSlug();
    if(dateActive(active)){
      const badge=$('q-lock-badge'); if(badge){badge.textContent=isSpanish()?'Abrir':'Open';badge.style.color='var(--red)';}
      const q=$('q-hub-card'); if(q) q.onclick=()=>{try{qUnlocked=true;}catch(e){}; if(typeof openQuestionnaire==='function') openQuestionnaire();};
    }
  }

  /* User hub: Watch Again tab/button to online version. */
  function onlineUrl(){return (isSpanish()?'/Espanol?online=1':'/online')+'?lesson='+encodeURIComponent(window.LESSON_SLUG||currentSlug());}
  function installWatchAgainHub(){
    const nav=document.querySelector('.hub-nav');
    if(nav && !$('watch-again-nav')){
      const a=document.createElement('a'); a.id='watch-again-nav'; a.href=onlineUrl(); a.textContent=isSpanish()?'VER OTRA VEZ':'WATCH AGAIN'; nav.appendChild(a);
    }
    const btns=document.querySelector('.hub-actions')||document.querySelector('.hub-btns');
    if(btns && !$('watch-again-hub-btn')){
      const b=document.createElement('button'); b.id='watch-again-hub-btn'; b.className='btn btn-outline'; b.textContent=isSpanish()?'VER OTRA VEZ':'WATCH AGAIN'; b.onclick=()=>{location.href=onlineUrl();}; btns.appendChild(b);
    }
  }

  /* Online notes stay in place. */
  function ensureNotesDrawer(){
    if($('online-notes-drawer')) return;
    const d=document.createElement('div'); d.id='online-notes-drawer'; d.className='online-notes-drawer';
    d.innerHTML='<div class="online-notes-head"><div class="online-notes-title" id="online-notes-title">Notes</div><button class="online-notes-close" id="online-notes-close">Close</button></div><textarea class="online-notes-ta" id="online-notes-ta" placeholder="Write your notes..."></textarea><div class="online-notes-help" id="online-notes-help">Saved on this device.</div>';
    document.body.appendChild(d);
    $('online-notes-close').onclick=()=>d.classList.remove('on');
    const ta=$('online-notes-ta');
    ta.value=localStorage.getItem('tm_online_notes_'+(window.LESSON_SLUG||currentSlug()))||'';
    ta.addEventListener('input',()=>localStorage.setItem('tm_online_notes_'+(window.LESSON_SLUG||currentSlug()),ta.value));
  }
  window.onlineOpenNotes=function(){
    ensureNotesDrawer();
    $('online-notes-title').textContent=isSpanish()?'Notas':'Notes';
    $('online-notes-close').textContent=isSpanish()?'Cerrar':'Close';
    $('online-notes-ta').placeholder=isSpanish()?'Escribe tus notas...':'Write your notes...';
    $('online-notes-help').textContent=isSpanish()?'Guardado en este dispositivo.':'Saved on this device.';
    $('online-notes-drawer').classList.add('on');
    setTimeout(()=>$('online-notes-ta').focus(),70);
  };

  /* Watch again should reuse current embedded player, not open another tab. */
  function patchWatchAgainInline(){
    const wa=$('online-watch-again'); if(!wa) return;
    wa.removeAttribute('target'); wa.removeAttribute('rel');
    wa.onclick=function(e){
      e.preventDefault();
      const frame=$('online-stream-iframe');
      const card=$('online-video-card');
      const cfg=(window.onlineViewerState&&window.onlineViewerState.config)||{};
      if(frame && cfg.embedUrl){
        const u=String(cfg.embedUrl);
        frame.src=u+(u.includes('?')?'&':'?')+'autoplay=1';
      }
  ...
  /* Online sync should use live cue immediately. Ignore stale saved delay for viewer cue sync. */
  const oldHandleOnline=window.handleOnlineMessage;
  if(oldHandleOnline && !oldHandleOnline.__v50){
    window.handleOnlineMessage=function(msg,hydrating){
      if(msg && (msg.type==='slide'||msg.type==='scripture')) msg.fast=true;
      if(window.onlineViewerState && window.onlineViewerState.config) window.onlineViewerState.config.syncDelaySeconds=0;
      const r=oldHandleOnline.call(this,msg,hydrating);
      setTimeout(()=>{try{patchWatchAgainInline(); if(typeof renderLowerThirdV49==='function') renderLowerThirdV49();}catch(e){}},20);
      return r;
    }; window.handleOnlineMessage.__v50=true;
  }

  /* If saved stream says ended, make Watch Again appear and inline-play. */
  const onlineRenderInterval=setInterval(()=>{if(document.body.classList.contains('online-viewer-mode')) patchWatchAgainInline();},1000);

  /* Bible bank chapter selector: force 2 Timothy 1,2,4 visible. */
  const oldRenderBible=window.renderBibleBankChapter;
  if(oldRenderBible && !oldRenderBible.__v50){
    window.renderBibleBankChapter=function(){
      const r=oldRenderBible.apply(this,arguments);
      setTimeout(addChapterPills,0);
      return r;
    }; window.renderBibleBankChapter.__v50=true;
  }
  function addChapterPills(){
    const bookSel=$('bb-book'), chapSel=$('bb-chapter'), tools=document.querySelector('.bible-bank-tools');
    if(!bookSel||!chapSel||!tools) return;
    const bank=window.BIBLE_BANK_CURRENT; const chapters=bank&&bank.books&&bank.books[bookSel.value]?Object.keys(bank.books[bookSel.value]):[];
    let wrap=$('bb-chapter-pills'); if(!wrap){wrap=document.createElement('div');wrap.id='bb-chapter-pills';wrap.className='bb-chapter-pills';tools.insertAdjacentElement('afterend',wrap);}
    wrap.innerHTML=chapters.map(c=>`<button type="button" class="bb-chapter-pill ${chapSel.value===c?'on':''}" data-ch="${c}">Chapter ${c}</button>`).join('');
    wrap.querySelectorAll('button').forEach(b=>b.onclick=()=>{chapSel.value=b.dataset.ch; if(oldRenderBible) oldRenderBible(); addChapterPills();});
  }
  const oldBuildBibleBank=window.buildBibleBank;
  if(oldBuildBible && !oldBuildBible.__v50){window.buildBibleBank=function(){const r=oldBuildBible.apply(this,arguments);setTimeout(addChapterPills,20);return r;}; window.buildBibleBank.__v50=true;}

  /* Timer starts when slide 2 is hit for the first time. */
  window._v50TimerStarted=false;
  const oldGoTo=window.goTo;
  if(oldGoTo && !oldGoTo.__v50){
    window.goTo=function(i){
      const r=oldGoTo.apply(this,arguments);
      if(Number(i)===1 && !window._v50TimerStarted){
        window._v50TimerStarted=true;
        try{_timerStart=Date.now(); ['cmd-timer','mm-timer','conf-timer'].forEach(id=>{const el=$(id); if(el) el.textContent='00:00';});}catch(e){}
      }
      return r;
    }; window.goTo.__v50=true;
  }

  /* Keep admin stream sync delay visually at zero unless the operator changes it intentionally. */
  function forceAdminDelayZero(){const d=$('stream-sync-delay'); if(d && (!d.dataset.touched)){d.value=0; d.oninput=()=>{d.dataset.touched='1';};}}

  document.addEventListener('DOMContentLoaded',()=>{
    fixAccessCaseNote(); setTimeout(fixAccessCaseNote,300); setTimeout(fixAccessCaseNote,1000);
    installWatchAgainHub(); unlockByDateUI(); translateSpanishParticipant();
    setTimeout(()=>{try{buildBibleBank&&buildBibleBank();addChapterPills();}catch(e){}},500);
    setTimeout(forceAdminDelayZero,900); setInterval(forceAdminDelayZero,2500);
    if(document.body.classList.contains('online-viewer-mode')||wantsOnline()){setTimeout(()=>{ensureNotesDrawer(); patchWatchAgainInline();},700);}
  });
})();
...
(function(){
  ...
  const esc=s=>String(s||'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const strip=html=>{const d=document.createElement('div');d.innerHTML=String(html||'');return (d.textContent||'').replace(/\s+/g,' ').trim();};
  const lesson=()=>{try{return normalizeLessonSlug(window.LESSON_SLUG||requestedLessonSlug()||'lesson-1');}catch(e){return window.LESSON_SLUG||'lesson-1';}};
  const isES=()=>route()==='espanol'||window.TM_LANG_ES||document.body.classList.contains('tm-spanish-mode');
  const isOnline=()=>route()==='online'||(route()==='espanol'&&/^(1|true|online)$/i.test(qs().get('online')||qs().get('view')||''));
  function onlineURL(lang){
    const l=encodeURIComponent(lesson());
    return lang==='es'?`/Espanol?online=1&lesson=${l}`:`/online?lesson=${l}`;
  }
  window.tmOnlineUrlV51=onlineURL;

  /* 1) Verse bank + Bible bank: one push updates P1 overlay and P2/scripture outputs. */
  function spanishRef(ref){return String(ref||'').replace(/1 Timothy/g,'1 Timoteo').replace(/2 Timothy/g,'2 Timoteo').replace(/Matthew/g,'Mateo');}
  window.pushVerseEverywhere=function(v){
    const ref=v.ref||v.ref_en||'';
    const kjv=v.kjv||v.text_en||v.text||'';
    const es=v.text_es||v.rvr||(typeof getRvrForRef==='function'?getRvrForRef(ref):'')||kjv;
    try{pushVerseToP1({ref:ref,kjv:kjv});}catch(e){}
    try{pushRawScripture({ref_en:ref,text_en:kjv,ref_es:spanishRef(ref),text_es:es});}catch(e){}
    try{if(typeof renderConfidenceScripture==='function') renderConfidenceScripture({ref_en:ref,text_en:kjv,ref_es:spanishRef(ref),text_es:es},'Active Scripture');}catch(e){}
  };
  function rebuildVerseButtons(){
    const wrap=$('verse-bank-cards');
    if(wrap && Array.isArray(window.VERSE_BANK||VERSE_BANK)){
      const bank=window.VERSE_BANK||VERSE_BANK;
      wrap.innerHTML=bank.map(v=>`<div class="vb-card" id="vbc-${esc(v.id)}"><div class="vb-ref">${esc(v.ref)}</div><div class="vb-text">${esc(v.kjv)}</div><div class="vb-actions"><button class="vb-btn p1" onclick="event.stopPropagation();pushVerseEverywhere({ref:'${esc(v.ref)}',kjv:\`${String(v.kjv||'').replace(/`/g,"'")}\`,text_es:getVBSpanish('${esc(v.id)}')});markVBPushed('${esc(v.id)}')">Push to Screens</button><button class="vb-btn p2" onclick="event.stopPropagation();pushRawScripture({ref_en:'${esc(v.ref)}',text_en:\`${String(v.kjv||'').replace(/`/g,"'")}\`,ref_es:'${esc(spanishRef(v.ref))}',text_es:getVBSpanish('${esc(v.id)}')});markVBPushed('${esc(v.id)}')">P2 Only</button></div></div>`).join('');
    }
  }
  const oldBuildVB=window.buildVerseBank;
  if(oldBuildVB&&!oldBuildVB.__v51){window.buildVerseBank=function(){const r=oldBuildVB.apply(this,arguments);setTimeout(rebuildVerseButtons,0);return r;};window.buildVerseBank.__v51=true;}
  const oldRenderBB=window.renderBibleBankChapter;
  if(oldRenderBB&&!oldRenderBB.__v51){window.renderBibleBankChapter=function(){const r=oldRenderBB.apply(this,arguments);setTimeout(()=>{
    document.querySelectorAll('#bible-bank-cards .bb-card').forEach(card=>{
      const ref=(card.querySelector('.bb-ref')||{}).textContent||'';
      const txt=(card.querySelector('.bb-text')||{}).textContent||'';
      const actions=card.querySelector('.bb-actions');
      if(actions) actions.innerHTML=`<button onclick="event.stopPropagation();pushVerseEverywhere({ref:'${esc(ref)}',kjv:\`${String(txt).replace(/`/g,"'")}\`})">Push to Screens</button><button onclick="event.stopPropagation();pushRawScripture({ref_en:'${esc(ref)}',text_en:\`${String(txt).replace(/`/g,"'")}\`,ref_es:'${esc(spanishRef(ref))}',text_es:getRvrForRef('${esc(ref)}')||\`${String(txt).replace(/`/g,"'")}\`})">P2 Only</button>`;
    });
  },0);return r;};window.renderBibleBankChapter.__v51=true;}

  /* 2) User lesson cards should select the lesson, not launch slideshow. */
  function showLessonDashboard(slug){
    try{applySelectedLesson(slug); updateLandingLessonState(slug);}catch(e){}
    try{document.querySelectorAll('.scr').forEach(x=>x.classList.remove('on')); const hub=$('sh'); if(hub) hub.classList.add('on');}catch(e){}
  }
  window.tmShowLessonDashboard=showLessonDashboard;
  const oldUpdate=window.updateLandingLessonState;
  if(oldUpdate&&!oldUpdate.__v51){window.updateLandingLessonState=function(slug){const r=oldUpdate.apply(this,arguments);setTimeout(()=>{
    document.querySelectorAll('.series-row .s-cell').forEach(cell=>{
      const n=((cell.textContent||'').match(/0?([1-4])\s+·/)||[])[1];
      if(n && !cell.classList.contains('locked')) cell.onclick=()=>showLessonDashboard('lesson-'+n);
    });
    const current=document.querySelector('.hub-body > .lcard:not(.locked)');
    if(current) current.onclick=()=>showLessonDashboard(window.LESSON_SLUG||slug||lesson());
    const top=$('watch-again-nav'); if(top) top.remove();
    const hub=$('watch-again-hub-btn'); if(hub) hub.onclick=()=>{location.href=onlineURL(isES()?'es':'en');};
  },0);return r;};window.updateLandingLessonState.__v51=true;}

  /* 3) Online language routing: ES in online mode goes to Spanish online, not Spanish dashboard. */
  function patchOnlineLangLinks(){
    const en=$('online-lang-en'), es=$('online-lang-es');
    if(en){en.href=onlineURL('en'); en.onclick=e=>{e.preventDefault(); location.href=onlineURL('en');};}
    if(es){es.href=onlineURL('es'); es.onclick=e=>{e.preventDefault(); location.href=onlineURL('es');};}
  }

  /* 4) Replace online current slide/on-screen with compact OBS-lower-third style + optional expanded points. */
  function slideInfo(){
    const slides=(window.LESSON1_SLIDES||LESSON1_SLIDES||[]);
    const idx=(window.onlineViewerState&&Number.isFinite(+window.onlineViewerState.slide))?+window.onlineViewerState.slide:(typeof curSlide==='number'?curSlide:0);
    const s=slides[Math.max(0,Math.min(idx,slides.length-1))]||{};
    let title='The Ministry', ref=(window.LESSON_DATA&&window.LESSON_DATA.text)||'Matthew 10', summary='';
    if(typeof slidePlainTitle==='function'){
      const p=slidePlainTitle(idx)||{}; title=p.title||title; ref=p.ref||ref; summary=p.summary||'';
    }else{
      title=strip(s.hl||s.text||s.title||title); ref=s.ref||ref; summary=(s.pts||[]).join(' ');
    }
    return {s,idx,title,ref,summary};
  }
  function ensurePointsCard(){
    if($('online-points-card')) return $('online-points-card');
    const card=document.createElement('section'); card.id='online-points-card'; card.className='online-points-card';
    card.innerHTML='<div class="online-points-title" id="online-points-title">Expanded Points</div><ul class="online-points-list" id="online-points-list"></ul>';
    const lower=$('online-lower-card');
    if(lower) lower.insertAdjacentElement('afterend',card);
    else ($('online-video-card')||$('online-viewer')).insertAdjacentElement('afterend',card);
    return card;
  }
  function renderOnlineOBSCard(){
    if(!document.body.classList.contains('online-viewer-mode')&&!isOnline()) return;
    const info=slideInfo();
    let lower=$('online-lower-card');
    if(!lower){lower=document.createElement('section'); lower.id='online-lower-card'; lower.className='online-lower-card'; ($('online-video-card')||$('online-viewer')).insertAdjacentElement('afterend',lower);}
    lower.innerHTML=`<div class="online-lower-obs"><div class="online-lower-obs-series">${esc(isES()?'El Ministerio · Mateo 10':'The Ministry · Matthew 10')}</div><div class="online-lower-obs-ref">${esc(info.ref||'')}</div><div class="online-lower-obs-text">${esc(info.title||'The Ministry')}</div>${info.summary?`<div class="online-lower-obs-summary">${esc(info.summary)}</div>`:''}<a class="online-watch-again" id="online-watch-again">${esc(isES()?'Ver otra vez':'Watch Again')}</a></div>`;
    const card=ensurePointsCard();
    const pts=Array.isArray(info.s.pts)?info.s.pts.map(strip).filter(Boolean):[];
    const list=$('online-points-list'); const title=$('online-points-title');
    if(title) title.textContent=isES()?'Puntos ampliados':'Expanded Points';
    if(pts.length){card.classList.add('on'); if(list) list.innerHTML=pts.map(p=>`<li>${esc(p)}</li>`).join('');}
    else{card.classList.remove('on'); if(list) list.innerHTML='';}
    patchWatchAgain();
  }
  function patchWatchAgain(){
    const wa=$('online-watch-again'); if(!wa) return;
    wa.href='#'; wa.removeAttribute('target'); wa.style.color='';
    const cfg=(window.onlineViewerState&&window.onlineViewerState.config)||{};
    const status=String(cfg.status||'').toLowerCase();
    wa.classList.toggle('on',status==='ended'||status==='replay');
    wa.textContent=isES()?'Ver otra vez':'Watch Again';
    wa.onclick=e=>{e.preventDefault(); const frame=$('online-video-frame'); if(frame){frame.scrollIntoView({behavior:'smooth',block:'start'}); const iframe=$('online-stream-iframe'); if(iframe&&iframe.src){const src=iframe.src; iframe.src='about:blank'; setTimeout(()=>iframe.src=src,60);}}};
  }
  const oldROS=window.renderOnlineSlide;
  if(oldROS&&!oldROS.__v51){window.renderOnlineSlide=function(){const r=oldROS.apply(this,arguments);setTimeout(renderOnlineOBSCard,0);return r;};window.renderOnlineSlide.__v51=true;}
  const oldROA=window.renderOnlineAll;
  if(oldROA&&!oldROA.__v51){window.renderOnlineAll=function(){const r=oldROA.apply(this,arguments);setTimeout(()=>{patchOnlineLangLinks();renderOnlineOBSCard();},0);return r;};window.renderOnlineAll.__v51=true;}
  const oldHOM=window.handleOnlineMessage;
  if(oldHOM&&!oldHOM.__v51){window.handleOnlineMessage=function(msg,hydrating){const r=oldHOM.apply(this,arguments);setTimeout(()=>{patchOnlineLangLinks();renderOnlineOBSCard();},25);return r;};window.handleOnlineMessage.__v51=true;}

  /* 5) Keep login note single. */
  function dedupeLowercaseNote(){
    const notes=[...document.querySelectorAll('#access-case-note-v50,.access-case-note-v50,#access-case-note-v51')];
    notes.forEach((n,i)=>{if(i>0)n.remove();});
    const ac=$('mo-access-code');
    if(ac && !document.querySelector('#access-case-note-v50,.access-case-note-v51')){const n=document.createElement('div');n.id='access-case-note-v51';n.className='access-case-note-v50';n.innerHTML='* access code is <b>all lowercase</b>';ac.appendChild(n);}
  }

  function boot(){
    try{rebuildVerseButtons(); if(window.renderBibleBankChapter) window.renderBibleBankChapter();}catch(e){}
    patchOnlineLangLinks(); renderOnlineOBSCard(); dedupeLowercaseNote();
    const top=$('watch-again-nav'); if(top) top.remove();
  }
  document.addEventListener('DOMContentLoaded',()=>{setTimeout(boot,250);setTimeout(boot,1100);});
  setInterval(()=>{if(isOnline()) {patchOnlineLangLinks(); renderOnlineOBSCard();} dedupeLowercaseNote();},1200);
})();

;

(function(){
  const escForAttr=s=>String(s||'').replace(/&/g,'&amp;').replace(/'/g,'&#39;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
  const escForTpl=s=>String(s||'').replace(/`/g,"'").replace(/\\/g,'\\\\');
  function spanishRef(ref){return String(ref||'').replace(/1 Timothy/g,'1 Timoteo').replace(/2 Timothy/g,'2 Timoteo').replace(/Matthew/g,'Mateo');}

  // Mobile verse-bank rows and mobile full Bible Bank must behave like desktop Push to Screens.
  // That means: P1/main projector overlay + P2 scripture screen + confidence/current scripture.
  function pushEverywhereMobile(ref, text, spanishText){
    if(typeof window.pushVerseEverywhere==='function'){
      window.pushVerseEverywhere({ref:ref,kjv:text,text_es:spanishText||''});
      return;
    }
    try{ if(typeof pushVerseToP1==='function') pushVerseToP1({ref:ref,kjv:text}); }catch(e){}
    try{ if(typeof pushRawScripture==='function') pushRawScripture({ref_en:ref,text_en:text,ref_es:spanishRef(ref),text_es:spanishText||text}); }catch(e){}
    try{ if(typeof renderConfidenceScripture==='function') renderConfidenceScripture({ref_en:ref,text_en:text,ref_es:spanishRef(ref),text_es:spanishText||text},'Active Scripture'); }catch(e){}
  }
  window.pushEverywhereMobile=pushEverywhereMobile;

  const oldPush=window.pushVerseFromMobile;
  window.pushVerseFromMobile=function(id,el=null){
    try{ if(typeof mobileFeedback==='function') mobileFeedback('verse',el); }catch(e){}
    const bank=window.VERSE_BANK||[];
    const v=bank.find(x=>x.id===id);
    if(!v){ try{ if(typeof mobileFeedback==='function') mobileFeedback('error',el); }catch(e){} return; }
    const es=(typeof getVBSpanish==='function'?getVBSpanish(v.id):'')||'';
    pushEverywhereMobile(v.ref,v.kjv,es);
  };

  window.renderMobileBibleBank=function(){
    const data=window.BIBLE_BANK_CURRENT||{};
    const books=data.books||{};
    const bookSel=document.getElementById('mm-bank-book');
    const chSel=document.getElementById('mm-bank-chapter');
    const list=document.getElementById('mm-bank-list');
    if(!bookSel||!chSel||!list) return;
    const bookNames=Object.keys(books);
    if(!bookNames.length){
      list.innerHTML='<div class="online-empty">No full chapter bank for this lesson.</div>';
      return;
    }
    if(!bookNames.includes(bookSel.value)){
      bookSel.innerHTML=bookNames.map(b=>`<option>${b}</option>`).join('');
      bookSel.value=bookNames[0];
    }
    const chapters=Object.keys(books[bookSel.value]||{});
    if(!chapters.includes(chSel.value)){
      chSel.innerHTML=chapters.map(c=>`<option>${c}</option>`).join('');
      chSel.value=chapters[0];
    }
    bookSel.onchange=()=>{chSel.value=''; window.renderMobileBibleBank();};
    chSel.onchange=window.renderMobileBibleBank;
    const verses=(books[bookSel.value]&&books[bookSel.value][chSel.value])||[];
    list.innerHTML=verses.map(([ref,txt])=>{
      const r=escForAttr(ref);
      const t=escForTpl(txt);
      const es=`${escForTpl((typeof getRvrForRef==='function'?getRvrForRef(ref):'')||'')}`;
      return `<button class="mm-bank-card" onclick="pushEverywhereMobile('${r}',\`${t}\`,\`${es}\`); if(window.mobileFeedback) mobileFeedback('verse',this)"><div class="ref">${escForAttr(ref)}</div><div class="txt">${escForAttr(txt)}</div><div class="mm-bank-push-note">Pushes to main projector + scripture screens</div></button>`;
    }).join('');
  };

  function rerenderIfMobileBankOpen(){
    if(document.body.classList.contains('mm-show-bank')) window.renderMobileBibleBank();
  }
  document.addEventListener('DOMContentLoaded',()=>setTimeout(rerenderIfMobileBankOpen,500));
})();
...
(function(){
  ...
  function fixAccessNoteOnce(){
    const ac=$('mo-access-code');
    if(!ac) return;
    [...ac.querySelectorAll('#access-case-note,#access-case-note-v50,#access-case-note-v51,.access-case-note,.access-case-note-v50,#access-code-single-note')].forEach(n=>n.remove());
    const note=document.createElement('div');
    note.id='access-code-single-note';
    note.innerHTML='* access code is <b>all lowercase</b>';
    ac.appendChild(note);
  }
  window.fixAccessNoteOnceV54=fixAccessNoteOnce;

  // Online pages on different Vercel URLs/origins do not get BroadcastChannel.
  // Realtime can lag, so online viewer gets a fast REST poll fallback.
  let v54LastSyncAt='';
  let v54PollingStarted=false;
  function normalizePayload(row){
    ...
    return payload&&payload.type?payload:null;
  }
  async function pollLatestCue(){
    try{
      if(!(document.body.classList.contains('online-viewer-mode') || (window.onlineViewerState&&window.onlineViewerState.active))) return;
      if(typeof loadSupabasePublicConfig==='function') await loadSupabasePublicConfig();
      if(!window.SB_URL||!window.SB_KEY) return;
      const res=await fetch(window.SB_URL+'/rest/v1/sync_state?id=eq.1&select=payload,updated_at&limit=1',{
        cache:'no-store',
        headers:{'apikey':window.SB_KEY,'Authorization':'Bearer '+window.SB_KEY,'Cache-Control':'no-cache'}
      });
      if(!res.ok) return;
      const rows=await res.json();
      const row=Array.isArray(rows)?rows[0]:null;
      if(!row||!row.updated_at||row.updated_at===v54LastSyncAt) return;
      v54LastSyncAt=row.updated_at;
      const msg=normalizePayload(row);
      if(!msg) return;
      // Force online cue changes to happen now. The video may be delayed, but the app UI should not be.
      if(msg.type==='slide'||msg.type==='scripture') msg.fast=true;
      if(typeof window.handleOnlineMessage==='function') window.handleOnlineMessage(msg,false);
      else if(typeof window.handleMessage==='function') window.handleMessage(msg);
    }catch(e){}
  }
  function startOnlineFastPoll(){
    if(v54PollingStarted) return;
    v54PollingStarted=true;
    setInterval(pollLatestCue,350);
    setTimeout(pollLatestCue,100);
    setTimeout(pollLatestCue,500);
  }

  // Remove any stored artificial online cue delay. Keep video itself untouched.
  const oldApply=window.applyOnlineConfig;
  if(oldApply && !oldApply.__v54){
    window.applyOnlineConfig=function(cfg){
      cfg=cfg||{}; cfg.syncDelaySeconds=0;
      const r=oldApply.call(this,cfg);
      try{ if(window.onlineViewerState&&window.onlineViewerState.config) window.onlineViewerState.config.syncDelaySeconds=0; }catch(e){}
      return r;
    };
    window.applyOnlineConfig.__v54=true;
  }
  const oldHandle=window.handleOnlineMessage;
  if(oldHandle && !oldHandle.__v54){
    window.handleOnlineMessage=function(msg,hydrating){
      if(msg && (msg.type==='slide'||msg.type==='scripture')) msg.fast=true;
      try{ if(window.onlineViewerState&&window.onlineViewerState.config) window.onlineViewerState.config.syncDelaySeconds=0; }catch(e){}
      return oldHandle.call(this,msg,hydrating);
    };
    window.handleOnlineMessage.__v54=true;
  }

  document.addEventListener('DOMContentLoaded',()=>{
    fixAccessNoteOnce();
    setTimeout(fixAccessNoteOnce,300);
    setTimeout(fixAccessNoteOnce,1200);
    if(location.pathname.toLowerCase().includes('online') || location.pathname.toLowerCase().includes('espanol')) startOnlineFastPoll();
  });
  setInterval(()=>{
    fixAccessNoteOnce();
    if(document.body.classList.contains('online-viewer-mode')) startOnlineFastPoll();
  },1500);
})();
...
(function(){
  function spanishRef(ref){return String(ref||'').replace(/1 Timothy/g,'1 Timoteo').replace(/2 Timothy/g,'2 Timoteo').replace(/Matthew/g,'Mateo');}
  function getEsForVerse(v){try{return (typeof getVBSpanish==='function'&&v&&v.id)?getVBSpanish(v.id):'';}catch(e){return '';}}
  function sendP1(ref,text){
    try{ if(typeof pushVerseToP1==='function') pushVerseToP1({ref:ref,kjv:text}); }catch(e){}
    // Force a network/local message even if the overlay was already open.
    try{ if(typeof sbSend==='function') sbSend({type:'p1_overlay_show',ref:ref,kjv:text,lesson:(window.LESSON_SLUG||'lesson-1'),ts:Date.now()}); }catch(e){}
  }
  function sendP2(ref,text,es){
    const payload={ref_en:ref,text_en:text,ref_es:spanishRef(ref),text_es:es||text};
    try{ if(typeof pushRawScripture==='function') pushRawScripture(payload); }catch(e){
      try{ if(typeof sbSend==='function') sbSend({type:'scripture',scripture:payload,lesson:(window.LESSON_SLUG||'lesson-1'),ts:Date.now()}); }catch(_){}
    }
    try{ if(typeof sbSend==='function') sbSend({type:'scripture',scripture:payload,lesson:(window.LESSON_SLUG||'lesson-1'),ts:Date.now()}); }catch(e){}
  }
  window.pushEverywhereMobile=function(ref,text,spanishText){
    ref=String(ref||''); text=String(text||''); spanishText=String(spanishText||'');
    sendP1(ref,text);
    sendP2(ref,text,spanishText);
    try{ if(typeof renderConfidenceScripture==='function') renderConfidenceScripture({ref_en:ref,text_en:text,ref_es:spanishRef(ref),text_es:spanishText||text},'Active Scripture'); }catch(e){}
  };
  window.pushVerseFromMobile=function(id,el){
    try{ if(typeof mobileFeedback==='function') mobileFeedback('verse',el); }catch(e){}
    const bank=window.VERSE_BANK||[];
    const v=bank.find(x=>x.id===id);
    if(!v){try{ if(typeof mobileFeedback==='function') mobileFeedback('error',el); }catch(e){} return;}
    window.pushEverywhereMobile(v.ref,v.kjv,getEsForVerse(v));
  };
  function patchMobileRows(){
    const wrap=document.getElementById('mm-verse-list');
    if(!wrap || !Array.isArray(window.VERSE_BANK)) return;
    wrap.innerHTML=window.VERSE_BANK.map(v=>`<button class="mm-v-btn" onclick="pushVerseFromMobile('${String(v.id).replace(/'/g,'&#39;')}',this)"><div class="mm-v-ref">${String(v.ref||'')}</div><div class="mm-v-tx">${String(v.kjv||'')}</div></button>`).join('');
  }
  const oldBuild=window.buildMobileVerseList;
  if(oldBuild && !oldBuild.__v56){
    window.buildMobileVerseList=function(){const r=oldBuild.apply(this,arguments);setTimeout(patchMobileRows,0);return r;};
    window.buildMobileVerseList.__v56=true;
  }
  document.addEventListener('DOMContentLoaded',()=>{setTimeout(patchMobileRows,300);setTimeout(patchMobileRows,1200);});
})();
