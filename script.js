...
(function(){
  ...
  window.renderPollResults=function(){
    const poll=currentPoll(); const box=document.getElementById('poll-results'); if(!box||!poll) return;
  ...
  window.handleMessage=function(msg){
    ...
    if(msg&&msg.type==='poll_close'){ window.pollState.active=null; window.pollState.selected=null; hidePollUI(); renderPollAdminSummary(); return; }
    if(msg&&msg.type==='poll_vote'){ addPollVote(msg.vote); return; }
    if(msg&&msg.type==='slide'){ hidePollUI(); window.pollState.active=null; }
    return previousHandle?previousHandle(msg):undefined;
  };
